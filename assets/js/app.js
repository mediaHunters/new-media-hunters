(function (window, document, $, undefined) {
  ...
  var axilInit = {
    ...
    w: function (e) {
      this._window.on("load scroll", function() {
        axilInit.checkAndInitSlider();
      }).on("scroll", axilInit.res);
    },
    

    checkAndInitSlider: function() {
      var sliderSection = $('.case-study-featured-area');
      if (sliderSection.length && !sliderInitialized) {
        var windowHeight = $(window).height();
        var scrollTop = $(window).scrollTop();
        var offsetTop = sliderSection.offset().top;
      ...
      function resizeClassAdd() {
        if (window.matchMedia("(min-width: 992px)").matches) {
          ...
          $("#mobilemenu-popup")
            ...
            .removeAttr("style");
          $(".axil-mainmenu c").remove();
          $(".axil-submenu").removeAttr("style");
        } else {
          $("body").addClass("mobilemenu-active").css("padding-right", "0");
